...
## 서론
여러분의 컴퓨터 작업 관리자를 열어보세요. CPU 그래프가 요동치며 수백 개의 프로세스가 돌아가고 있을 겁니다. 우리 자바 애플리케이션도 마찬가지입니다. 사용자가 동시에 수천 명씩 몰려드는데, 요리사가 단 한 명뿐인 주방처럼 한 번에 한 주문만 처리한다면 그 서비스는 금방 망하고 말겠죠.

![CPU가 100퍼센트 찍고 비명을 지르는 짤](/images/02_Back-end/java_concurrency/cpu_screaming_100.png)
*(여기에 '본체가 이륙할 것 같은 소음'이나 '불타는 CPU' 짤을 추천합니다!)*
...
![동시성 vs 병렬성 개념 도식](/images/02_Back-end/java_concurrency/concurrency_vs_parallelism_diagram.png)
*(한 명의 작업자가 여러 일을 스위칭하는 모습과, 여러 작업자가 각자 일을 수행하는 모습을 대조한 기술 도식을 넣어주세요!)*
...
- 자바 성능 튜닝 이야기 (이상민 저)
---
*(여기에 '질서 정연하게 돌아가는 톱니바퀴'나 '평온한 숲' 짤을 넣어 마무리하면 좋습니다!)*
