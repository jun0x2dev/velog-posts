...
## 서론
프로젝트를 진행하다 보면 "아, 이 코드 또 쓰고 있네"라는 자괴감에 빠질 때가 있습니다. 모든 메서드 시작과 끝에 로그를 남기거나, 성능을 측정하기 위해 실행 시간을 계산하는 코드를 넣을 때가 대표적이죠. 비즈니스 로직은 단 3줄인데, 앞뒤로 붙는 부가적인 코드가 10줄이라면 배보다 배꼽이 더 큰 상황이 됩니다.

![반복되는 로그 코드에 고통받는 개발자 짤](/images/02_Back-end/spring_magic/boilerplate_code_hell.png)
*(여기에 '복사 붙여넣기를 반복하다가 영혼이 가출한 개발자'나 '똑같은 벽돌을 수천 개 쌓는 사람' 짤을 추천합니다!)*

이런 노가다(?)를 멈추기 위해 등장한 것이 바로 **AOP(Aspect-Oriented Programming)**입니다. 오늘은 코드 곳곳에 흩어진 공통 관심사를 한데 모아 관리하는 이 우아한 기술과, 그 뒤에서 묵묵히 일하는 **프록시(Proxy)**의 정체를 밝혀보겠습니다.
...
![AOP 프록시 동작 원리 도식](/images/02_Back-end/spring_magic/aop_proxy_mechanism_diagram.png)
*(Client가 Proxy를 호출하고, Proxy가 Advice를 실행한 뒤 Target 메서드를 호출하는 흐름도를 넣어주세요!)*
...
---
*(여기에 '무대 뒤에서 일하는 스태프들'이나 '깔끔하게 분리된 레이어' 짤을 넣어 마무리하면 좋습니다!)*
